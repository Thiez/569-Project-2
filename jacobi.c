...
extern void jacobi(long n, long m, REAL dx, REAL dy, REAL alpha, REAL relax, REAL * u_p, REAL * f_p, REAL tol, int mits);
...
int main(int argc, char * argv[]) {
  ...
  num_threads = omp_get_num_threads();
        ...
        printf("================================= OpenMP Execution (%d threads) ======================================\n", num_threads);
...
void jacobi_seq(long n, long m, REAL dx, REAL dy, REAL alpha, REAL omega, REAL * u_p, REAL * f_p, REAL tol, int mits) {
	...
	while ((k <= mits) && (error > tol)) {
		...
		memcpy(uold, u, n*m*sizeof(u[0][0]));
		/*
		for (i = 0; i < n; i++)
			for (j = 0; j < m; j++)
				uold[i][j] = u[i][j];
...
void jacobi_omp(long n, long m, REAL dx, REAL dy, REAL alpha, REAL omega, REAL * u_p, REAL * f_p, REAL tol, int mits) {
  ...
  REAL (*f)[m] = (REAL(*)[m])f_p;
	...
	// We cannot parallelize at this level because successive answers depend on
	// their predecessors.
  while ((k <= mits) && (error > tol)) {
    error = 0.0;
    
    /* Copy new solution into old */
    // Note that memory copying is IO-bound, and thus does not benefit from
    // parallelization. Best-case is using the system library for memory copying.
    //
    // Possible optimization: use a variable to count iteration parity and just
    // swap the variables used. This would require a bit of a hack to avoid
    // taking a performance hit from the branch predictor, i.e., store u as a
    // 2xMxN array, using u[parity & 1] and u[~parity & 1] to address u and
    // u_old.
    memcpy(uold, u, n*m*sizeof(u[0][0]));
    ...
    for (i = 1; i < (n - 1); ++i) {
      for (j = 1; j < (m - 1); ++j) {
        float resid = (ax * (uold[i - 1][j] + uold[i + 1][j]) +
                 ay * (uold[i][j - 1] + uold[i][j + 1]) + b *
                 uold[i][j] - f[i][j]) / b;
